...
class _WebViewContainerState extends State<WebViewContainer> {
  ...
  Future<void> _triggerFileInput(String inputId, String base64Image, String fileName, String fileType) async {
    String script = '''
  (function() {
    try {
      console.log("Activando input file: " + "$inputId");
      
      // Primero, intentar encontrar el input
      let fileInput = null;
      ...
      if ("$inputId" === "pago_moviles_peru_trf_image") {
        ...
        // Buscar cualquier input de tipo file visible
      ...
      if (!fileInput) {
        console.error("No se pudo encontrar el input file");
        return;
      }
      
      console.log("Input encontrado: " + fileInput.id);
      ...
      // ENFOQUE DIRECTO: Asignar el archivo al input
      try {
        // Este es el método estándar que debería funcionar en la mayoría de navegadores modernos
        const dataTransfer = new DataTransfer();
        dataTransfer.items.add(file);
        fileInput.files = dataTransfer.files;
        
        // Disparar múltiples eventos para asegurar que se detecta el cambio
        // 1. Evento change estándar
        ...
        // 3. Si tiene un manejador onchange
        if (fileInput.hasAttribute('onchange')) {
          const onchangeAttr = fileInput.getAttribute('onchange');
          console.log("Atributo onchange detectado: " + onchangeAttr);
          
          // Intentar ejecutar el código onchange
          if (onchangeAttr && onchangeAttr.includes('prepareImage')) {
            console.log("Ejecutando función prepareImage");
            try {
              // Esto es una solución alternativa que ejecuta el código directamente
              // Usamos eval con precaución ya que es la única forma de ejecutar
              // código dinámico en este contexto
              let code = onchangeAttr.replace('this', 'fileInput');
              eval(code);
              console.log("Código onchange ejecutado manualmente");
            } catch (err) {
              console.error("Error ejecutando código onchange: " + err);
            }
          }
        }
        
        console.log("Archivo asignado exitosamente al input");
        
        // Feedback visual (si es posible)
        if (fileInput.parentElement) {
          // Si hay un elemento de texto cerca para mostrar el nombre
          const textElements = fileInput.parentElement.querySelectorAll('span, p, label');
          textElements.forEach(function(elem) {
            if (!elem.hasAttribute('for') || elem.getAttribute('for') === fileInput.id) {
              elem.textContent = "Archivo seleccionado: " + file.name;
            }
          });
        }
